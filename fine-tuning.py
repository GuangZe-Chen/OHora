...
class SavePeftModelCallback(transformers.TrainerCallback):
    def __init__(self,interval_ratio=0.1):
        ...
        #self.param_names = ['q_proj','v_proj',"k_proj","down_proj.base_layer","up_proj"]
        #self.param_names = ['q_proj.lora_A','q_proj.lora_B',"v_proj.lora_A","v_proj.lora_B"]
        '''
        self.param_names = ['q_proj.lora_A','q_proj.base_layer','q_proj.lora_B','v_proj.base_layer',"v_proj.lora_A","v_proj.lora_B",'k_proj.base_layer',"k_proj.lora_A","k_proj.lora_B"
                             ,"down_proj.lora_A","down_proj.lora_B",'down_proj.base_layer',"up_proj.lora_A","up_proj.lora_B",'up_proj.base_layer']
        '''
        ...
        for i in range(32):
            ...
            self.params_history[i] = {'q_proj.lora_A':0,'q_proj.lora_B':0,"v_proj.lora_A":0,"v_proj.lora_B":0,"k_proj.lora_A":0,"k_proj.lora_B":0,
                            "down_proj.lora_A":0,"down_proj.lora_B":0,"up_proj.lora_A":0,"up_proj.lora_B":0,'q_proj.base_layer':0,'k_proj.base_layer':0,'v_proj.base_layer':0,'up_proj.base_layer':0,'down_proj.base_layer':0}
            '''
            self.params_history[i] = {'q_proj.lora_A':0,'q_proj.lora_B':0,"v_proj.lora_A":0,"v_proj.lora_B":0,"k_proj.lora_A":0,"k_proj.lora_B":0,
                                      "down_proj.lora_A":0,"down_proj.lora_B":0,"up_proj.lora_A":0,"up_proj.lora_B":0}
            #self.params_history[i] = {"q_proj.base_layer":0,"v_proj.base_layer":0,"k_proj.base_layer":0,"down_proj.base_layer":0,"up_proj.base_layer":0}
    ...
    def on_step_end(self, args, state, control, **kwargs):
        ...
        if step > 0 and step % self.interval == 0:
            print(f"[Callback] Step {step}: Performing SVD-based LoRA reinitialization.")
    ...
    def on_train_end(self, args, state, control, **kwargs):
        self.model = kwargs["model"]
        if isinstance(kwargs["model"],PeftModel):
            output_path = "weight_outputs/lora/ohora_r_64.pth"
            #self.record_weight_change_history(output_path)
            if state.log_history:
                for log in state.log_history:
                     if "loss" in log:
                        self.losses.append(log["loss"])
            save_loss_path = 'training_loss/pissa/ohora_std_loss.pth' 
    ...
    def record_weight_change_history(self,output_path=None):
        ...
        print(f"Fine-tuned Weight has been saved in {output_path}") 
...
def main():
    ...
    if model_args.tokenizer_name:
        tokenizer = AutoTokenizer.from_pretrained(model_args.tokenizer_name, **tokenizer_kwargs)   #llama2
    elif model_args.tokenizer_name_or_path:
        #tokenizer = LlamaTokenizer.from_pretrained(model_args.tokenizer_name_or_path, **tokenizer_kwargs)
        #tokenizer = AutoTokenizer.from_pretrained(model_args.tokenizer_name_or_path, **tokenizer_kwargs)  #llama2
        #/**********llama3**********/

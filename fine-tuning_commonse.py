...
class SavePeftModelCallback(transformers.TrainerCallback):
    def __init__(self):
        ...
        self.losses = []
        self.steps = []
        #self.param_names = ['q_proj.lora_A','q_proj.lora_B',"v_proj.lora_A","v_proj.lora_B","k_proj.lora_A","k_proj.lora_B","o_proj.lora_A","o_proj.lora_B","gate_proj.lora_A","gate_proj.lora_B","down_proj.lora_A","down_proj.lora_B","up_proj.lora_A","up_proj.lora_B"]
        self.param_names = ['q_proj.lora_A','q_proj.lora_B',"v_proj.lora_A","v_proj.lora_B","k_proj.lora_A","k_proj.lora_B",
                            "down_proj.lora_A","down_proj.lora_B","up_proj.lora_A","up_proj.lora_B"]
        for i in range(32):
            #self.params_history[i] = {'q_proj.lora_A':0,'q_proj.lora_B':0,"v_proj.lora_A":0,"v_proj.lora_B":0,"k_proj.lora_A":0,"k_proj.lora_B":0,"o_proj.lora_A":0,"o_proj.lora_B":0,"gate_proj.lora_A":0,"gate_proj.lora_B":0,"down_proj.lora_A":0,"down_proj.lora_B":0,"up_proj.lora_A":0,"up_proj.lora_B":0}
            self.params_history[i] = {'q_proj.lora_A':0,'q_proj.lora_B':0,"v_proj.lora_A":0,"v_proj.lora_B":0,"k_proj.lora_A":0,"k_proj.lora_B":0,
                            "down_proj.lora_A":0,"down_proj.lora_B":0,"up_proj.lora_A":0,"up_proj.lora_B":0}
    
    def on_train_begin(self,args,state,control,**kwargs):
        self.model = kwargs['model']
        if isinstance(kwargs["model"],PeftModel):
            output_files = "weight_outputs/commonsense/dora_std_ori.pth"
    ...
    @contextmanager
    def _temporary_saveable_generation_config(self, model):
        generation_config = getattr(model, "generation_config", None)
        if generation_config is None or getattr(generation_config, "do_sample", None) is not False:
            yield
            return

        default_generation_config = generation_config.__class__()
        ...
        original_values = {
            field_name: getattr(generation_config, field_name)
            for field_name in sampling_only_fields
            if hasattr(generation_config, field_name)
        }
        try:
            for field_name in sampling_only_fields:
                if hasattr(generation_config, field_name):
                    setattr(generation_config, field_name, getattr(default_generation_config, field_name))
            yield
        finally:
            for field_name, value in original_values.items():
                setattr(generation_config, field_name, value)

    def _save_model_and_processor(self, model, processing_class, save_directory):
        with self._temporary_saveable_generation_config(model):
            model.save_pretrained(save_directory)
        processing_class.save_pretrained(save_directory)
        self._cleanup_saved_jsons(save_directory)

    def _cleanup_saved_jsons(self, save_directory: str) -> None:
        ...
        try:
            ...
            # Drop optional sampling-only keys to keep saved config minimal and avoid downstream confusion.
            for key in ["temperature", "top_p", "top_k", "min_p", "typical_p", "epsilon_cutoff", "eta_cutoff"]:
                generation_config.pop(key, None)
            with open(generation_config_path, "w", encoding="utf-8") as f:
                json.dump(generation_config, f, indent=2, ensure_ascii=True)
                f.write("\n")
        except Exception as exc:
            logger.warning(f"skip generation_config cleanup for {save_directory}: {exc}")
    ...
    def save_model(self, args, state, kwargs):
        ...
        self._save_model_and_processor(kwargs["model"], kwargs["processing_class"], peft_model_path)
    ...
    def on_train_end(self, args, state, control, **kwargs):
        self.model = kwargs["model"]
        if isinstance(kwargs["model"],PeftModel):
            output_path = "weight_outputs/commonsense/olora_std_trained.pth"
            #self.record_weight_change_history(output_path)
            if state.log_history:
                for log in state.log_history:
                     if "loss" in log:
                        self.losses.append(log["loss"])
            save_loss_path = 'training_loss/commonsense/olora_std_loss.pth' 
            #torch.save({"train_loss":self.losses,"log_steps":self.steps}, save_loss_path)
            kwargs["model"] = kwargs["model"].merge_and_unload()
            logger.info("OHoRA training finished: adapter has been merged into base model before final save.")
        peft_model_path = os.path.join(args.output_dir, "sft_lora_model")
        self._save_model_and_processor(kwargs["model"], kwargs["processing_class"], peft_model_path)
        logger.info(f"Saved merged full model to: {peft_model_path}")
...
class MyTrainingArguments(TrainingArguments):
    ...
    ohora_init_method: Optional[str] = field(default="classic")
    ohora_mix_alpha: Optional[float] = field(default=0.5)
    ...
    skip_resize_token_embeddings: Optional[bool] = field(default=False)
    paper_repro_mode: Optional[bool] = field(default=False)
...
def _peft_debug_enabled() -> bool:
    return os.environ.get("OHORA_PEFT_DEBUG", "0") == "1"


def _stage_debug_enabled() -> bool:
    return os.environ.get("OHORA_STAGE_DEBUG", "0") == "1"


def _stop_after_trainer_create() -> bool:
    return os.environ.get("OHORA_STOP_AFTER_TRAINER_CREATE", "0") == "1"


def _log_stage(stage: str) -> None:
    if _stage_debug_enabled():
        logger.info(f"[stage-debug] {stage}")
...
def _dataset_uses_token_id(dataset, token_id: int) -> bool:
    if dataset is None:
        return False
    for sample in dataset:
        for field_name in ("input_ids", "labels"):
            ...
            if torch.is_tensor(values):
                if torch.any(values == token_id).item():
                    return True
                continue
...
def _paper_repro_resize_skip_reason(training_args, tokenizer, train_dataset, eval_dataset, model_vocab_size: int):
    if not (training_args.paper_repro_mode and training_args.use_ohora):
        return None
    if model_vocab_size + 1 != len(tokenizer):
        return None
    extra_token_id = len(tokenizer) - 1
    extra_token = tokenizer.convert_ids_to_tokens(extra_token_id)
    if extra_token != "<pad>":
        return None
    if _dataset_uses_token_id(train_dataset, extra_token_id) or _dataset_uses_token_id(eval_dataset, extra_token_id):
        return None
    return {
...
def main():
    ...
    if training_args.paper_repro_mode and training_args.use_ohora and not _cli_has_flag("--trainable"):
        training_args.trainable = "q_proj,v_proj"

    default_cache_dir = _default_cache_dir()
    if model_args.cache_dir is None and default_cache_dir is not None:
        model_args.cache_dir = default_cache_dir
    if data_args.data_cache_dir is None and default_cache_dir is not None:
        data_args.data_cache_dir = os.path.join(default_cache_dir, "datasets")
    if training_args.output_dir == "trainer_output":
        output_root = os.environ.get("OHORA_OUTPUT_ROOT")
        if output_root:
            training_args.output_dir = os.path.join(output_root, "commonsense_ohora")

    require_version("datasets>=1.8.0", "To fix: pip install -r examples/pytorch/language-modeling/requirements.txt")

    import datasets as hf_datasets
    from build_dataset_reasoning import build_commonsense_reasoning_instruction_dataset, DataCollatorForSupervisedDataset
    ...
    hf_datasets.utils.logging.set_verbosity(log_level)
    ...
    if model_args.tokenizer_name:
        ...
        '''
        if training_args.paper_repro_mode:
            logger.info(
                "paper_repro_mode enabled: keeping tokenizer padding configuration from checkpoint "
                f"(pad_token={tokenizer.pad_token}, pad_token_id={tokenizer.pad_token_id})"
            )
        else:
            #llama-3.1-8B
    ...
    if training_args.do_train:
        with training_args.main_process_first(desc="loading and tokenization"):
            ...
            train_dataset = build_commonsense_reasoning_instruction_dataset(
                data_path=files,
                tokenizer=tokenizer,
    ...
    if model_vocab_size != len(tokenizer):
        paper_repro_resize_skip = _paper_repro_resize_skip_reason(
            training_args,
            tokenizer,
            train_dataset,
            eval_dataset,
        ...
        if paper_repro_resize_skip is not None:
            logger.warning(
                "Skipping resize_token_embeddings in paper_repro_mode: "
                f"model vocab size={model_vocab_size}, tokenizer length={len(tokenizer)}, "
                f"extra token={paper_repro_resize_skip['extra_token']} "
                f"(id={paper_repro_resize_skip['extra_token_id']})"
            )
        elif training_args.skip_resize_token_embeddings:
            logger.warning(
                "Skipping resize_token_embeddings for 1-token mismatch localization: "
                f"model vocab size={model_vocab_size}, tokenizer length={len(tokenizer)}"
            )
        else:
    ...
    if training_args.peft_path is not None: # --------------------------> train from the trained lora model
        ...
        if training_args.use_pissa:
            ...
            method = (training_args.ohora_init_method or "classic").lower()
            if method == "classic":
                init_lora_weights = "ohora"
            elif method in ["relative", "relative_scores", "ohora_relative_scores"]:
                init_lora_weights = "ohora_relative_scores"
            elif method in ["mix", "relative_scores_mix", "ohora_relative_scores_mix"]:
                init_lora_weights = "ohora_relative_scores_mix"
            elif method in ["classic_mix_rerank", "classic_rerank", "ohora_classic_mix_rerank"]:
                init_lora_weights = "ohora_classic_mix_rerank"
            else:
                raise ValueError(
                    f"Unsupported ohora_init_method={training_args.ohora_init_method}. "
                    "Use one of: classic, relative_scores, relative_scores_mix, classic_mix_rerank"
                )
        ...
        peft_config = LoraConfig(
            ...
            ohora_mix_alpha=training_args.ohora_mix_alpha,
            hash_r=hash_r
            )
        _log_stage("LoraConfig built")
        _log_stage("get_peft_model:start")
        model = get_peft_model(model, peft_config)
        _log_stage("get_peft_model:return")
        end_time = time.time()
    ...
    _log_stage("print_trainable_parameters:start")
    model.print_trainable_parameters()
    _log_stage("print_trainable_parameters:return")
    ...
    _log_stage("trainer_create:start")
    trainer = Trainer(
        model=model,
        args=training_args,
    ...
    _log_stage("trainer_create:return")
    if _stop_after_trainer_create():
        logger.info("[stage-debug] stop_after_trainer_create")
        return

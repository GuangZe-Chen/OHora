...
RUN_ROOT="${RUN_ROOT:-/nas_data/xueyue.yang/ohora_runs/requested_l2_l3_commonsense_bs2_lr7e4_$(date +%Y%m%d_%H%M%S)}"
...
case "${1:-both}" in
  l2)
    run_one \
      "LLaMA-2-7B_lr7e-4_bs2_wr0.01_e3" \
      "/nas_data/xueyue.yang/hf_models/Llama-2-7b-hf" \
      "${RUN_ROOT}/llama2_7b_lr7e-4_bs2_wrr0.01_e3" \
  ...
  l3)
    run_one \
      "LLaMA-3-8B_lr7e-4_bs2_ws30_e3" \
      "/nas_data/xueyue.yang/hf_models/Meta-Llama-3-8B" \
      "${RUN_ROOT}/llama3_8b_lr7e-4_bs2_ws30_e3" \
      "${GPU_CANDIDATES:-${2:-0 1 2 3 4 5 6 7}}" \
      30 \
      0 \
      False

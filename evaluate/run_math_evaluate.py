...
if torch.cuda.is_available():
    gpu_id = 3  #Specify the GPU number when using a single GPU
...
def main(
    ...
    for instruction in [
        ...
        print()
    """
    save_file = f'experiment/{args.model}-math-{args.dataset}.json'
...
def load_data(args) -> list:
    ...
    """
    file_path = f'datasets/test/{args.dataset}/test.json'
...
def parse_args():
    ...
    parser.add_argument('--dataset', choices=['AddSub', 'MultiArith', 'SingleEq', 'gsm8k', 'AQuA', 'SVAMP'],
                        default='gsm8k',required=False)
    parser.add_argument('--model', default='Llama-3.1',choices=['LLaMA-7B', 'BLOOM-7B', 'GPT-j-6B'], required=False)
    parser.add_argument('--adapter', default='',choices=['LoRA', 'AdapterP', 'AdapterH', 'Parallel', 'Prefix'],
                        required=False)
    parser.add_argument('--base_model', default='outputs/Llama-2-7B/OHoRA_reinit/lora_model/sft_lora_model',required=False)
...
def load_model(args) -> tuple:
    ...
    if "LLaMA" in args.model:
        tokenizer = LlamaTokenizer.from_pretrained(base_model,torch_dtype=torch.bfloat16)
    else:
        tokenizer = AutoTokenizer.from_pretrained(base_model,torch_dtype=torch.bfloat16)
